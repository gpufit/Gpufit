...
void generate_gauss_1d(std::vector< double > & v, std::vector< double > const & p)
{
	...
	{
		double const argx = ((i - p[1]) * (i - p[1])) / (2.f * p[2] * p[2]);
...
void generate_gauss_2d(std::vector< double > & v, std::vector< double > const & p)
{
    ...
    {
        double const argy = ((j - p[2]) * (j - p[2]));
        for (std::size_t i = 0; i < n; i++)
        {
            double const argx = ((i - p[1]) * (i - p[1]));
            double const ex = exp(-(argx + argy) / (2.f * p[3] * p[3]));
            v[j * n + i] = p[0] * ex + p[3];
        }
    }
}
...
void generate_gauss_2d_elliptic(std::vector< double > & v, std::vector< double > const & p)
{
    ...
    for (std::size_t j = 0; j < n; j++)
    {
        double const argy = ((j - p[2]) * (j - p[2])) / (2.f * p[4] * p[4]);
        for (std::size_t i = 0; i < n; i++)
        {
            double const argx = ((i - p[1]) * (i - p[1])) / (2.f * p[3] * p[3]);
