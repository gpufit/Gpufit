...
if __name__ == '__main__':
    ...
    spline_fit_initial_parameters = true_fit_parameters + np.array([amp_shift, pos_shift_x * SF, pos_shift_y * SF, off_shift], dtype=np.float32)
    ...
    linear_noisy_psf_gauss = np.reshape(noisy_psf[:, 2:-2], (1, 24**2))  # this will only work if size_x,y aren't changed above (try to cut off data symmetrical to make a square data array for gauss fit)
    
    # call to gpufit with spline fit
    parameters_spline, states_spline, chi_squares_spline, n_iterations_spline, time_spline = gf.fit(linear_noisy_psf, None, gf.ModelID.SPLINE_2D, spline_fit_initial_parameters, tolerance, max_n_iterations, None, estimator_id, user_info)
    if not np.all(states_spline == 0):
        raise RuntimeError('Not all spline fits converged.')

    # call to gpufit with gauss1d fit
    parameters_gauss, states_gauss, chi_squares_gauss, n_iterations_gauss, time_gauss = gf.fit(linear_noisy_psf_gauss, None, gf.ModelID.GAUSS_2D, gauss_fit_initial_parameters, tolerance, max_n_iterations, None, estimator_id)
