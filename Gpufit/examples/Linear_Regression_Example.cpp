...
void linear_regression_example()
{
    ...
    */
	...
	std::vector< double > user_info(n_points_per_fit);
	for (size_t i = 0; i < n_points_per_fit; i++)
	{
		user_info[i] = static_cast<double>(pow(2, i));
	}

	// size of user info in bytes
	size_t const user_info_size = n_points_per_fit * sizeof(double); 
	...
	// true parameters
	std::vector< double > true_parameters { 5, 2 }; // offset, slope
	...
	for (size_t i = 0; i != data.size(); i++)
	{
		...
		double x = user_info[k];
		double y = true_parameters[0] + x * true_parameters[1];
