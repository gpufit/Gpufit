...
void patlak_two()
{
	...
	*/
	...
	size_t const n_fits = 100; // 10000? 100?
	...
	REAL time[] = { 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75, 4, 4.25, 4.5, 4.75, 5,
					5.25, 5.5, 5.75, 6, 6.25, 6.5, 6.75, 7, 7.25, 7.5, 7.75, 8, 8.25, 8.5, 8.75, 9, 9.25, 9.5, 9.75, 10,
					10.25, 10.5, 10.75, 11, 11.25, 11.5, 11.75, 12, 12.25, 12.5, 12.75, 13, 13.25, 13.5, 13.75, 14, 14.25, 14.5, 14.75, 15 };		// in min. up to 15 min
	...
	{
		user_info[i] = static_cast<REAL>(time[i]);
		std::cout << user_info[i] << std::endl;
	}
	std::cout << "Time filled..." << std::endl;
	...
	std::cout << "Checking user_info vector..." << std::endl;
	for (size_t i = 0; i < 2 * n_points_per_fit; i++)
	{
		std::cout << user_info[i] << std::endl;
	}

	std::cout << user_info.size() << std::endl;
	std::cout << user_info.max_size() << std::endl;
	...
	// true parameters
	std::vector< REAL > true_parameters{ 0.005, 0.02 }; // Ktrans, vp -- ?
	...
	for (size_t i = 0; i != n_fits; i++)
	{
		...
		initial_parameters[i * n_model_parameters + 1] = true_parameters[0] * (0.8f + 0.4f * uniform_dist(rng));
			...
			std::cout << std::endl << "parameters randomized..." << std::endl;

	// generate data -- NEEDS TO BE UPDATED WITH PATLAK MODEL
	...
	for (size_t i = 0; i != data.size(); i++)
	{
		...
		REAL x = 0;
		for (int n = 1; n < k; n++) {				// or is point_index i's limit? ya probably
		// 
			REAL spacing = time[n] - time[n - 1];
			x += Cp[n] * spacing + 0.5 * (Cp[n] - Cp[n - 1]);
		}
		//REAL x = user_info[k];			 // how to define w/ 2 x's? integral of Cp(k) to point_index?
		REAL y = true_parameters[0] * x + true_parameters[1] * user_info[k + n_points_per_fit];
		data[i] = y + normal_dist(rng);
	}

	// tolerance
	REAL const tolerance = 0.001f;		// calculated by Christian to be 10e-15f
	...
	(
		...
		output_number_iterations.data()
			...
			std::cout << std::endl << "gpufit complete." << std::endl;
	...
	if (status != ReturnState::OK)
	{
		throw std::runtime_error(gpufit_get_last_error());
			...
			std::cout << std::endl << "status is ok." << std::endl;
...
{
	std::cout << std::endl << "Beginning Patlak fit...v2" << std::endl;
	patlak_two();
