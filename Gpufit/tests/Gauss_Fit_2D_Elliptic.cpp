...
void generate_gauss_2d_elliptic(std::array< double, SIZE>& values, std::array< double, 6 > const & parameters)
{
    ...
    for (int point_index_y = 0; point_index_y < size_y; point_index_y++)
    {
        for (int point_index_x = 0; point_index_x < size_x; point_index_x++)
        {
            ...
            double const argx = ((point_index_x - x0)*(point_index_x - x0)) / (2.f * sx * sx);
            double const argy = ((point_index_y - y0)*(point_index_y - y0)) / (2.f* sy * sy);
