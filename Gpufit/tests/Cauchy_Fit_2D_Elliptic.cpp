...
void generate_cauchy_2d_elliptic(std::array< double, SIZE>& values, std::array< double, 6 > const & parameters)
{
    ...
    double const y0 = parameters[2];
    double const sx = parameters[3];
    double const sy = parameters[4];
    double const b = parameters[5];
    ...
    for (int point_index_y = 0; point_index_y < size_y; point_index_y++)
    {
        for (int point_index_x = 0; point_index_x < size_x; point_index_x++)
        {
            ...
            double const argx = ((x0 - point_index_x) / sx) *((x0 - point_index_x) / sx) + 1.f;
            double const argy = ((y0 - point_index_y) / sy) *((y0 - point_index_y) / sy) + 1.f;
...
BOOST_AUTO_TEST_CASE( Cauchy_Fit_2D_Elliptic )
{
    ...
    std::array< double, 6 > const true_parameters{ { 4.f, 2.f, 2.f, 0.4f, 0.6f, 1.f } };
    ...
    std::array< double, 6 > initial_parameters{ { 2.f, 1.8f, 2.2f, 0.5f, 0.5f, 0.f } };
    ...
    std::array< double, 6 > output_parameters;
    int output_state;
