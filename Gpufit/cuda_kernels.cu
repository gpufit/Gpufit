...
{
    Device_Array<int> n_parameters_gpu(1);
    Device_Array<int> n_dimensions_gpu(1);
    cuda_configure_model <<< 1, 1 >>> (model_id, n_parameters_gpu, n_dimensions_gpu);

    n_parameters_gpu.copy(1, n_parameters);
    n_dimensions_gpu.copy(1, n_dimensions);
}

__global__ void cuda_configure_model(int const model_id, int * n_parameters, int * n_dimensions)
{
    device_configure_model(model_id, n_parameters, n_dimensions);
}

void configure_estimator(int const estimator_id)
{
    cuda_configure_estimator <<< 1, 1 >>> (estimator_id);
}

__global__ void cuda_configure_estimator(int const model_id)
{
    device_configure_estimator(model_id);
}
    ...
    std::size_t const user_info_size)
{
    ...
    calculate_model(current_parameters, n_fits, n_points, current_values, current_derivatives, point_index, fit_index, chunk_index, user_info, user_info_size);
}
