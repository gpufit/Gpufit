#ifndef GPUFIT_SPLINE4D_MULTICHANNEL_CUH_INCLUDED
#define GPUFIT_SPLINE4D_MULTICHANNEL_CUH_INCLUDED

/* Description of the calculate_spline4d_multichannel function
* ============================================================
*
* This function calculates a value of a four-dimensional spline model function
* with multiple channels and its partial derivatives with respect to the model parameters.
*
* No independent variables are passed to this model function.  Hence, the
* (X, Y, Z, W, CH) coordinate of the first data value is assumed to be (0.0, 0.0, 0.0, 0.0, 0).
* For a fit size of M x N x O x P x Q data points, the (X, Y, Z, W, CH) coordinates of the data
* are simply the corresponding array index values of the data array, starting from zero.
*
* Parameters:
*
...
*                user_info[4]: the number of data points in W
...
*                user_info[9]:  the value of coefficient (0,0,0,0) of interval (0,0,0,0,0)
*                user_info[10]: the value of coefficient (1,0,0,0) of interval (0,0,0,0,0)
*                   .
*                   .
*                   .
*                user_info[13]: the value of coefficient (0,1,0,0) of interval (0,0,0,0,0)
*                   .
*                   .
*                   .
*                user_info[265]: the value of coefficient (0,0,0,0) of interval (1,0,0,0,0)
*                   .
*                   .
*                   .
*                user_info[9+Ni*256]: the value of coefficient (0,0,0,0) of intervall (0,0,0,0,1)
*                   (Ni: number of intervals per channel)
*                   .
*                   .
*                   .
*
...
*
* Calling the calculate_spline4d_multichannel function
* ====================================================
*
...
__device__ void calculate_spline4d_multichannel(
    ...
    std::size_t const user_info_size)
{
    ...
    std::size_t const n_points_w = static_cast<std::size_t>(*(user_info_REAL + 4));
    ...
    std::size_t const n_intervals_per_channel = n_intervals_x * n_intervals_y * n_intervals_z * n_intervals_w;
    std::size_t const n_coefficients_per_interval = 256;
    REAL const * coefficients = user_info_REAL + 9;

    // parameters
    REAL const * p = parameters;

    // estimate coordinates (i, j, k, q) of the current interval
    ...
    int q = static_cast<int>(floor(position_w));

    // adjust i, j k, and q to their bounds
    i = i >= 0 ? i : 0;
    ...
    q = q >= 0 ? q : 0;
    q = q < n_intervals_w ? q : n_intervals_w - 1;
    ...
    std::size_t const interval_index
        = channel_id * n_intervals_per_channel
    ...
    REAL const w_diff = position_w - q;
    ...
    for (int order_i = 0; order_i < 4; order_i++)
    {
        ...
        for (int order_j = 0; order_j < 4; order_j++)
        {
            ...
            {
                REAL power_factor_q = 1;
                for (int order_q = 0; order_q < 4; order_q++)
                {
                    // intermediate function value without amplitude and offset
                    temp_value
                        += current_coefficients[order_i * 64 + order_j * 16 + order_k * 4 + order_q]
                        * power_factor_i
                        * power_factor_j
                        * power_factor_k
                        * power_factor_q;
                    ...
                    if (order_i < 3)
                    {
                        temp_derivative_1
                            += (REAL(order_i) + 1)
                            * current_coefficients[(order_i + 1) * 64 + order_j * 16 + order_k * 4 + order_q]
                            * power_factor_i
                            * power_factor_j
                            * power_factor_k
                            * power_factor_q;
                    ...
                    if (order_j < 3)
                    {
                        temp_derivative_2
                            += (REAL(order_j) + 1)
                            * current_coefficients[order_i * 64 + (order_j + 1) * 16 + order_k * 4 + order_q]
                            * power_factor_i
                            * power_factor_j
                            * power_factor_k
                            * power_factor_q;
                    ...
                    if (order_k < 3)
                    {
                        temp_derivative_3
                            += (REAL(order_k) + 1)
                            * current_coefficients[order_i * 64 + order_j * 16 + (order_k + 1) * 4 + order_q]
                            * power_factor_i
                            * power_factor_j
                            * power_factor_k
                            * power_factor_q;
                    ...
                    if (order_q < 3)
                    {
                        temp_derivative_4
                            += (REAL(order_q) + 1)
                            * current_coefficients[order_i * 64 + order_j * 16 + order_k * 4 + (order_q + 1)]
                            * power_factor_i
                            * power_factor_j
                            * power_factor_k
                            * power_factor_q;
