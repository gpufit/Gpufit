...
*
* The reduced triexponential function is: S/S0 = (1-a)*exp(-b*x) + a*exp(-c*x)
* The derivatives are:
    ...
    std::size_t const user_info_size)
{
    ...
    current_derivatives[1 * n_points] = (p[0] - 1) * x * exp(-p[1] * x);
    current_derivatives[2 * n_points] = -p[0] * x * exp(-p[2]*x);
}
