#ifndef ESR3111_CUH_INCLUDED
#define ESR3111_CUH_INCLUDED
...
__device__ void calculate_esr3111(
    ...
    std::size_t const user_info_size)
{
    ...
    // value
	// params [a1 a2 a3 fmid gamma offset]
	//         p0 p1 p2  p3    p4     p5 
	// frequencies (x values) must be given in MHz
	// N14 hyperfine constant is taken to be 2.158 MHz
	
	float const dHF = 2.158;
	float const arg1 = (x-p[3]+dHF)/(p[4]/2);
	float const arg2 = (x-p[3]    )/(p[4]/2);
	float const arg3 = (x-p[3]-dHF)/(p[4]/2);
	...
	float const denom3 = 1/(1 + arg3sq);
	
	
    value[point_index] = p[0] * denom1 + p[1]*denom2 + p[2]*denom3 + p[5];
	...
	current_derivative[2 * n_points]  = denom3;
    current_derivative[3 * n_points]  = p[0]*4.f*denom1*denom1*arg1/p[4] + p[1]*4.f*denom2*denom2*arg2/p[4] + p[2]*4.f*denom3*denom3*arg3/p[4];
	current_derivative[4 * n_points]  = p[0]*2.f*denom1*denom1*arg1sq/p[4] + p[1]*2.f*denom2*denom2*arg2sq/p[4] + p[2]*2.f*denom3*denom3*arg3sq/p[4];
