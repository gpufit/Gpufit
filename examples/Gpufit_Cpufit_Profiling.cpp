    ...
    std::chrono::milliseconds::rep const duration_in_ms)
{
	...
	double n_converged = 0.0;
	for (int i = 0; i < n_fits; i++)
		if (states[i] == 0)
			n_converged += 1.0;

	std::cout << std::setprecision(6);
	std::cout << "converged: " << n_converged/double(n_fits)*100.0 << " %" << std::endl;
}
...
void generate_initial_parameters(std::vector<REAL> & parameters_set, std::vector<Parameters> const & parameters)
{
    ...
    REAL const offset = 0.3f;

    int const n_parameters = sizeof(Parameters) / sizeof(REAL);

    std::size_t  const n_fits = parameters_set.size() / n_parameters;

    for (std::size_t i = 0; i < n_fits; i++)
    {
        parameters_set[0 + i * n_parameters] = parameters[i].amplitude * (1 + 2 * offset * (uniform_dist(rng) - 0.5f));
        parameters_set[1 + i * n_parameters] = parameters[i].center_x + parameters[i].width * 2 * offset * (uniform_dist(rng) - 0.5f);
        parameters_set[2 + i * n_parameters] = parameters[i].center_y + parameters[i].width * 2 * offset * (uniform_dist(rng) - 0.5f);
        parameters_set[3 + i * n_parameters] = parameters[i].width * (1 + 2 * offset * (uniform_dist(rng) - 0.5f));
        parameters_set[4 + i * n_parameters] = parameters[i].background * (1 + 2 * offset * (uniform_dist(rng) - 0.5f));
    }
}
...
void generate_test_parameters(std::vector<Parameters> & target, Parameters const source)
{
    ...
    REAL const center_offset = 1.f;

    for (std::size_t i = 0; i < n_fits; i++)
    {
        target[i].amplitude = source.amplitude;
        target[i].center_x = source.center_x + (2 * center_offset * (uniform_dist(rng) - 0.5f));
        target[i].center_y = source.center_y + (2 * center_offset * (uniform_dist(rng) - 0.5f));
        target[i].width = source.width;
        target[i].background = source.background;
    }
}
...
{
    REAL const std_dev = parameters.amplitude / (snr* logf(10.f));

    std::normal_distribution<REAL> distribution(0.0, std_dev);
...
int main(int argc, char * argv[])
{
    ...
    // data size
    std::size_t const n_fits = 5000000;
    std::size_t const n_fits_cpu = 5000000;
	...
	true_parameters.center_x = static_cast<REAL>(size_x) / 2.f - 0.5f;
	true_parameters.center_y = static_cast<REAL>(size_x) / 2.f - 0.5f;
	true_parameters.width = 1.f;
	true_parameters.background = 10.f;

	//  test parameters
	std::cout << "generate test parameters" << std::endl;
	std::vector<Parameters> test_parameters(n_fits);
	generate_test_parameters(test_parameters, true_parameters);

	//  test data
	std::vector<REAL> data(n_fits * n_points);
	generate_gauss2d(n_fits, n_points, data, test_parameters);
	std::cout << "add noise" << std::endl;
	add_gauss_noise(data, true_parameters, 60.f);

	// initial parameter set
	std::vector<REAL> initial_parameters(n_parameters * n_fits);
	generate_initial_parameters(initial_parameters, test_parameters);

	std::cout << std::endl;
	std::cout << n_fits << " fits" << std::endl;
	...
	{
		// print results
		print_result("Gpufit", gpufit_parameters, test_parameters, gpufit_states, gpufit_n_iterations, n_fits, n_parameters, dt_Gpufit);
	}
