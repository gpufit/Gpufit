...
void print_result(
    ...
    std::vector<double> const & estimated_parameters,
    ...
    std::chrono::milliseconds::rep const duration_in_ms)
{
    ...
    std::vector<double> estimated_x_centers(n_fits);
    std::vector<double> test_x_centers(n_fits);
...
int main(int argc, char * argv[])
{
	...
	//  test data
	std::vector<double> data(n_fits_gpu * n_points);
	...
	std::vector<double> initial_parameters(n_parameters * n_fits_gpu);
	...
	// Cpufit output
	std::vector<double> cpufit_parameters(n_fits_cpu * n_parameters);
	std::vector<int> cpufit_states(n_fits_cpu);
	std::vector<double> cpufit_chi_squares(n_fits_cpu);
	...
	std::vector<double> gpufit_parameters(n_fits_gpu * n_parameters);
	std::vector<int> gpufit_states(n_fits_gpu);
	std::vector<double> gpufit_chi_squares(n_fits_gpu);
