    ...
    std::chrono::milliseconds::rep const duration_in_ms)
{
    ...
    float const std_dev_x = calculate_standard_deviation(estimated_x_centers, test_x_centers, states);

    float const mean_n_iterations = calculate_mean(n_iterations, states);

    float fits_per_second = static_cast<float>(n_fits) / duration_in_ms * 1000;
...
int main(int argc, char * argv[])
{
    ...
    std::cout << "\nPERFORMANCE GAIN Gpufit/Cpufit \t" << std::setw(10) << static_cast<float>(dt_cpufit) / dt_gpufit * n_fits_gpu / n_fits_cpu << std::endl;
