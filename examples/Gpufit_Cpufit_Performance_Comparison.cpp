...
int main(int argc, char * argv[])
{
    ...
    for (std::size_t fit_index = 0; fit_index < n_fits_all.size(); fit_index++)
    {
        ...
        if (dt_cpufit)
        {
            std::cout << std::setw(13) << static_cast<float>(n_fits) / static_cast<float>(dt_cpufit)* 1000.0 << std::setw(3) << "|";
        }
        else
        {
        ...
        if (dt_gpufit)
        {
            std::cout << std::setw(13) << static_cast<float>(n_fits) / static_cast<float>(dt_gpufit)* 1000.0 << std::setw(3) << "|";
            std::cout << std::fixed << std::setprecision(2);
            std::cout << std::setw(12) << static_cast<float>(dt_cpufit) / static_cast<float>(dt_gpufit);
        }
        else if (!do_gpufits)
        {
